  ...
  return (
    ...
    >
      ...
      <button type="button" className="min-w-0 flex-1 text-left" onClick={onOpen}>
        ...
        <p className="tnum text-[12px] text-ink-3">{setSummary(row.item.sets)}</p>
      </button>
    </Reorder.Item>
  );
}
...
export function TemplateEditorScreen({ templateId }: { templateId: string }) {
  ...
  return (
    <Screen bottomPadding={40}>
      ...
      >
        <IconPlus size={18} /> Ajouter un exercice
      </Pressable>
          ...
          save([
            ...rows,
            {
              ...
              item: {
                ...
                sets: e.isTimeBased
                  ? [
                      { type: 'hold', durationSec: 20 },
                      { type: 'hold', durationSec: 20 },
                    ]
                  : [
                      { type: 'normal', repsMin: 8, repsMax: 12 },
                      { type: 'normal', repsMin: 8, repsMax: 12 },
                    ],
              },
            },
        ...
        {editing && editIdx !== null && (
          <div className="pb-3 pt-1">
            ...
            <p className="mb-4 text-[13px] text-ink-2">
              Tape le type de série pour le changer.
            </p>

            {editing.item.sets.map((s, si) => {
              const cycle = () => {
                const next =
                  TYPE_CYCLE[(TYPE_CYCLE.indexOf(s.type) + 1) % TYPE_CYCLE.length];
            ...
            <Pressable
              className="mt-2.5 flex w-full items-center justify-center gap-1.5 rounded-[12px] bg-raised-2 py-2.5 text-[14px] font-semibold text-ink-2"
              onClick={() =>
                patchItem(editIdx, {
                  sets: [
                    ...editing.item.sets,
